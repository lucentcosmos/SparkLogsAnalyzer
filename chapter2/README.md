...
loading data and exporting that is suitable for a production system.  In particular, the techniques must scale to handle large production volumes of logs data.

To scale, Apache Spark is meant to be deployed on a cluster of machines.  While you
could continue running the examples in local mode on data sets that are small
enough to fit in memory on one machine, it is recommended
that you set up a simple one node cluster on your local machine and run
the remaining examples on that local cluster - just to get practice
working with a cluster.  See this page on the
